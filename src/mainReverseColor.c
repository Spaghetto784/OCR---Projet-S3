...
int main(int argc, char *argv[])
{
    ...
    SDL_Surface *reverse_image = invert_colors(grayscale_image);
    SDL_FreeSurface(grayscale_image);    
    

    SDL_Surface *bw_image = convert_to_bw(reverse_image, threshold);
    SDL_FreeSurface(reverse_image); // Free the grayscale image after conversion
    ...
    SDL_FreeSurface(bw_image); // Free the black and white image after rotation
    ...
    while (!quit)
    {
        ...
        {
            if (event.type == SDL_QUIT) // Quit if the user closes the window
            {
                quit = 1;
            }
            if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE) // Quit if the user presses Escape
            {
                quit = 1;
            }
        }
