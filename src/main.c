...
int main(int argc, char *argv[]) {
  ...
  SDL_FreeSurface(
      grayscale_image); // Libérer l'image en niveaux de gris après rotation
  ...
  SDL_Window *window = SDL_CreateWindow(
      "Image Rotatée", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
  ...
  SDL_FreeSurface(
      rotated_image); // Libérer la surface après création de la texture
  ...
  // Boucle d'événements pour garder la fenêtre ouverte jusqu'à ce que
    ...
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_KEYDOWN &&
          event.key.keysym.sym ==
              SDLK_ESCAPE) { // Quitter si l'utilisateur appuie sur Échap
        quit = 1;
      }
    }
  }
