...
void convert_to_grayscale(SDL_Surface *surface) {
  if (surface->format->BytesPerPixel != 3 &&
    ...
    printf(
        "Format d'image non supporté pour la conversion en niveaux de gris.\n");
    return;
  }
...
SDL_Surface *rotate_image(SDL_Surface *surface, int angle) {
  ...
  // Parcourir chaque pixel de la nouvelle surface et déterminer sa couleur
  // d'après la surface d'origine
  for (int y = 0; y < new_height; y++) {
    for (int x = 0; x < new_width; x++) {
      // Calculer la position inversement transformée
      int original_x =
          (int)((x - ncx) * cos(-radians) - (y - ncy) * sin(-radians) + cx);
      int original_y =
          (int)((x - ncx) * sin(-radians) + (y - ncy) * cos(-radians) + cy);

      // Vérifier si la position est dans les limites de l'image originale
      if (original_x >= 0 && original_x < original_width && original_y >= 0 &&
          original_y < original_height) {
        // Obtenir la couleur du pixel d'origine
        Uint32 pixel =
            ((Uint32 *)
                 surface->pixels)[original_y * original_width + original_x];
