...
SDL_Surface *load_image(const char *path) {
  SDL_Surface *img = IMG_Load(
      path); // Utilisation de SDL_image pour charger différents formats
  if (!img) {
...
void rotate_image(SDL_Surface *img, double angle) {
  ...
  if (rotated_img) {
    SDL_FreeSurface(img);
    img =
        rotated_img; // Vous devrez gérer la mise à jour de la surface d'origine
  }
}
...
void enhance_contrast(SDL_Surface *img) {
  ...
  for (int i = 0; i < img->w * img->h; i++) {
    ...
    r = (Uint8)fmin(255,
                    fmax(0, r * 1.2)); // Exemples d'augmentation de contraste
